...
namespace websockets {
    ...
    void WebsocketsClient::upgradeToSecuredConnection() {
        ...
        if(this->_optional_ssl_ca_cert) {
            ...
            // setInsecure() in ArduinoWebsockets 0.5.4 only clears the
            // optional CA pointer; it forgot to configure the newly-created
            // WiFiClientSecure. Without this call ESP32 aborts TLS with
            // start_ssl_client: -1 before the WebSocket handshake.
            client->setInsecure();
        }
