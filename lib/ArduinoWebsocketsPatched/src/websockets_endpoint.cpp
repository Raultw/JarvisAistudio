...
namespace internals {
    ...
    WebsocketsFrame WebsocketsEndpoint::_recv() {
        ...
        // A successful read is allowed to drain the TCP buffer completely.
        // Testing available() here discarded valid frames whenever no later
        // WebSocket frame had arrived yet (large Gemini audio frames hit this
        // consistently). Validate the number of bytes actually read instead.
